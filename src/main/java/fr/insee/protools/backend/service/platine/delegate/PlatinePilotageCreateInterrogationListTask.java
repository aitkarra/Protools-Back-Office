...
public class PlatinePilotageCreateInterrogationListTask implements JavaDelegate, DelegateContextVerifier {
    ...
    public void execute(DelegateExecution execution) {
        ...
        for (ProtoolsInterrogationDto interrogationDto : interrogationList){
            //Create the platine DTO object
            QuestioningWebclientDto dto = computeQuestioningWebclientDto(interrogationDto.getRemInterrogation(),
                    interrogationDto.getWebConnectionId(),//TODO: a terme fait partie de l'interrogation
                    currentPartitionNode,campainId);
        ...
        log.info("ProcessInstanceId={}  campainId={} - currentPartitionId={} - end ",
                execution.getProcessInstanceId(), campainId,currentPartitionId);
    }

    private static QuestioningWebclientDto computeQuestioningWebclientDto(JsonNode remSUNode, String webConnectionId,JsonNode currentPartitionNode, String idCampagne) {
        ...
        ContactAccreditationDto platineContact = convertREMPersonToPlatineContact(webConnectionId, platineAdress, contact);
    ...
    private static ContactAccreditationDto convertREMPersonToPlatineContact(String idInternaute, PlatineAddressDto platineAdress, PersonDto remPersonDto) {
        ...
        return ContactAccreditationDto.builder()
                ...
                .email(remPersonDto.getEmails().stream()
                        .filter(email -> email.getFavorite()!=null && email.getFavorite())
                        .findFirst()
                        .orElse(remPersonDto.getEmails().stream().findFirst().orElse(new EmailDto()))
                        .getMailAddress())
                .phone(
                        remPersonDto.getPhoneNumbers().stream()
                                .filter(phoneNumber -> phoneNumber.getFavorite()!=null && phoneNumber.getFavorite())
                                .findFirst()
